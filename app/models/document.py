...
class DocumentChunk(Base):
    ...
    # jieba 分词后的空格串（中文按词切分，英文按空白切分）。供 PG tsvector 稀疏检索使用（to_tsvector('simple', search_text) 上的 GIN 表达式索引）。为 NULL 表示该 chunk 尚未建立搜索文本；SQLite 开发环境无 PG GIN 索引，稀疏检索会回退内存 BM25。
